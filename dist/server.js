...
const config = JSON.parse(_fs2.default.readFileSync(_path2.default.resolve("dist/config.json"))); /**
...
config.map(node => {
	console.log(node);
	if ((0, _typeCheck.typeCheck)('Number', node.port)) {
		if (ports.indexOf(node.port) == -1) {
			ports.push(node.port);
		}
	} else if ((0, _typeCheck.typeCheck)('[Number]', node.port)) {
		node.port.map(port => {
